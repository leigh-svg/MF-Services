...
function drawDiagram(doc,system,componentStates,startY){
  ...
  rect(doc,margin,startY,dW,dH,[245,245,245]);
  rect(doc,margin,startY,dW,8,[225,225,225]);
  ...
  inc.forEach((c,i)=>{
    ...
    rect(doc,bx,bY,bW,bH,c.mandatory?[245,220,220]:[240,240,240]);
    ...
    line(doc,cx,bY+bH,cx,bY+bH+4,[200,200,200],0.4);
  });
...
export async function generateCablePlanPDF({system,componentStates,projectData}){
  ...
  rect(doc,0,0,pageW,8,[235,235,235]);
  ...
  if(system.isFireDoor){
    const badgeW=30,badgeH=6,badgeX=pageW-margin-badgeW,badgeY=26;
  ...
  if(meta.length===0){doc.setFont("helvetica","italic");doc.setFontSize(8);doc.setTextColor(...C.silver);txt(doc,"No project data provided.",margin+4,y+5);y+=10;}
    ...
    meta.forEach(([label,value],i)=>{
      ...
      if(col===0)rect(doc,fx,fy,cW,10,row%2===0?[245,245,245]:[238,238,238]);
  ...
  rect(doc,margin,y,cW,7,[230,230,230]);doc.setFont("helvetica","bold");doc.setFontSize(6.5);doc.setTextColor(40,40,40);
  ...
  allComps.forEach(({comp,depth},idx)=>{
    ...
    rect(doc,margin,y,cW,rowH,isMandatory?[255,235,235]:idx%2===0?[245,245,245]:[235,235,235]);
  ...
  rect(doc,margin,y,cW,20,[245,250,245]);line(doc,margin,y,margin+cW,y,[34,153,84],1);
  ...
  for(let p=1;p<=totalPages;p++){
    doc.setPage(p);rect(doc,0,pageH-10,pageW,10,[235,235,235]);
