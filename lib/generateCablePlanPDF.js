...
const C = {
  navy:[60,60,60],blue:[60,90,130],lightBlue:[120,150,200],red:[180,50,50],
  green:[34,153,84],silver:[120,120,120],mist:[200,200,200],white:[255,255,255],
};
...
function drawDiagram(doc,system,componentStates,startY){
  ...
  rect(doc,margin,startY,dW,dH,[255,255,255]);
  rect(doc,margin,startY,dW,8,[255,255,255]);
  ...
  if(inc.length>1)line(doc,sX+bW/2,bY+bH/2,sX+tW-bW/2,bY+bH/2,C.lightBlue,0.5);
  inc.forEach((c,i)=>{
    const bx=sX+i*(bW+gX),cx=bx+bW/2;
    rect(doc,bx,bY,bW,bH,c.mandatory?[255,240,240]:[255,255,255]);
    doc.setFont("helvetica","bold");doc.setFontSize(7);doc.setTextColor(40,40,40);
    txt(doc,c.position,cx,bY+6,{align:"center"});
    doc.setFont("helvetica","normal");doc.setFontSize(5.5);doc.setTextColor(...C.silver);
    ...
    if(cable){doc.setFontSize(5);doc.setTextColor(...C.navy);txt(doc,cable,cx,bY+bH+5,{align:"center"});}
    line(doc,cx,bY+bH,cx,bY+bH+4,[200,200,200],0.1);
  });
...
export async function generateCablePlanPDF({system,componentStates,projectData}){
  ...
  rect(doc,0,0,pageW,8,[0,0,128]);
  doc.setFont("helvetica","bold");doc.setFontSize(14);doc.setTextColor(255,255,255);
  txt(doc,"CABLE PLAN",margin+10,14);
  doc.setFont("helvetica","normal");doc.setFontSize(8);doc.setTextColor(255,255,255);
  txt(doc,"MF Services — Door Systems Configuration",margin+10,22);
  // TODO: Embed MF Services logo here - add doc.addImage('path/to/logo.png', 'PNG', x, y, w, h);
  const now=new Date(),dateStr=now.toLocaleDateString("en-IE",{day:"2-digit",month:"short",year:"numeric"});
  txt(doc,"Generated: "+dateStr,pageW-margin,14,{align:"right"});
  doc.setFont("helvetica","bold");doc.setFontSize(10);doc.setTextColor(255,255,255);
  txt(doc,system.name+" · "+system.leafType,pageW-margin,24,{align:"right"});
  if(system.isFireDoor){
    const badgeW=30,badgeH=6,badgeX=pageW-margin-badgeW,badgeY=30;
    rect(doc,badgeX,badgeY,badgeW,badgeH,C.red);
    doc.setFont("helvetica","bold");doc.setFontSize(6);doc.setTextColor(...C.white);
    txt(doc,"FIRE DOOR",badgeX+badgeW/2,badgeY+4,{align:"center"});
  }
  let y=36;
  rect(doc,margin,y,cW,8,[255,255,255]);doc.setFont("helvetica","bold");doc.setFontSize(7);doc.setTextColor(40,40,40);
  ...
  if(meta.length===0){doc.setFont("helvetica","italic");doc.setFontSize(8);doc.setTextColor(...C.silver);txt(doc,"No project data provided.",margin+4,y+5);y+=10;}
    ...
    meta.forEach(([label,value],i)=>{
      ...
      if(col===0)rect(doc,fx,fy,cW,10,row%2===0?[255,255,255]:[250,250,250]);
      doc.setFont("helvetica","normal");doc.setFontSize(6.5);doc.setTextColor(80,80,80);txt(doc,label.toUpperCase(),fx+4,fy+4);
      doc.setFont("helvetica","bold");doc.setFontSize(8);doc.setTextColor(40,40,40);txt(doc,value||"—",fx+4,fy+8.5);
    });
  ...
  rect(doc,margin,y,cW,8,[245,245,245]);doc.setFont("helvetica","bold");doc.setFontSize(7);doc.setTextColor(40,40,40);
  ...
  rect(doc,margin,y,cW,7,[255,255,255]);doc.setFont("helvetica","bold");doc.setFontSize(6.5);doc.setTextColor(40,40,40);
  ...
  allComps.forEach(({comp,depth},idx)=>{
    ...
    if(y+rowH>pageH-20){doc.addPage();y=20;rect(doc,margin,y,cW,7,[230,230,230]);doc.setFont("helvetica","bold");doc.setFontSize(6.5);doc.setTextColor(40,40,40);Object.values(cols).forEach(col=>txt(doc,col.label,col.x+2,y+5));y+=7;}
    rect(doc,margin,y,cW,rowH,isMandatory?[255,235,235]:idx%2===0?[255,255,255]:[250,250,250]);
    if(isMandatory)rect(doc,margin,y,2,rowH,C.red);else if(depth>0)rect(doc,margin,y,2,rowH,[160,160,160]);
    const tY=y+4;
    ...
    y+=rowH;line(doc,margin,y,margin+cW,y,[20,40,70],0.1);
  });
  y+=6;if(y>pageH-30){doc.addPage();y=20;}
  rect(doc,margin,y,cW,20,[255,255,255]);line(doc,margin,y,margin+cW,y,[34,153,84],0.5);
  ...
  for(let p=1;p<=totalPages;p++){
    doc.setPage(p);rect(doc,0,pageH-10,pageW,10,[255,255,255]);
